...
- **Continuous Scroll Mode (Phase 1-2):** `ContinuousScrollContentView` with fit-to-width layout, parallel image size preloading via `TaskGroup`, index tracking callback. `ImageScrollView.reflectScrolledClipView` delegates to `updateVisibleSlots`. `ImageWindowController.animateResize` uses CADisplayLink for smooth window resize when scrolling to different-sized images. `ImageLoader.getImageSize(for:)` returns EXIF-aware dimensions. Toggle via Navigation menu. `folder.currentIndex` synced during scroll.
- **Continuous Scroll Mode (Phases 1-2):** Webtoon-style vertical scrolling. `ContinuousScrollContentView` uses standard macOS coordinates (y=0 at bottom) to match `ImageScrollView`'s unflipped behavior. Binary search (O(log n)) for index tracking. `scaledHeights` cache avoids redundant calculations. CADisplayLink for smooth window resize with center-preserving animation. `ImageLoader.getImageSize(for:)` handles EXIF orientation swap for orientations 5-8.
