...
- **Manual zoom is session state; `alwaysFitOnOpen` is a preference** — `enterManualZoom()` sets `isManualZoom` but does NOT touch `alwaysFitOnOpen` (menu checkbox = open-behavior preference, stays checked). `isManualZoom` is deliberately not decoded in `ViewerSettings.init(from:)` (launch always starts in fit) and is reset in `loadFolder` (new folder / Finder re-open → fit). Within the same folder, page turns preserve manual magnification (`applyFitting` checks `isManualZoom` BEFORE `alwaysFitOnOpen` — manga reading requirement; do not reorder). ⌘0 (`fitOnScreen`) clears `isManualZoom` and fully restores auto-fit. `manualZoomInheritsFitWindowResize` still preserves window hugging during manual zoom from auto-fit; clear it when returning to fit. Do not collapse this back to a simple `isManualZoom`/`alwaysFitOnOpen` binary.
- **Open-fit denominator is the screen, not the window** — `openFitViewport()` returns `window.contentRect(forFrameRect: screen.visibleFrame)` (minus status bar) when auto-fit + window-hug are active and not fullscreen; otherwise falls back to `effectiveScrollViewport`. `applyFitting` uses it so images are never squashed into the provisional window size. `handleWindowDidResize` deliberately keeps the current-viewport denominator (the dragged size is the user's choice) — do not unify the two.
- **createWindow pre-sizes single-page windows** — reads the image header via `ImageLoader.imageHeaderSize(at:)` (public sync, EXIF 5-8 swap, PDF → nil) and sets the hugging frame via `presizedContentSize` + `FittingCalculator` before `showWindow` (no visible resize jump). Gate uses global settings only — per-folder dual-page overrides load later in `viewDidAppear`, so a mismatch just falls back to the old flow. Any failure (unreadable header, no screen) falls back to `savedOrDefaultWindowSize()`.
...
- **Xee-style open fitting (root fix):** Fit denominator on open changed from the provisional window to the screen's usable content (`openFitViewport()`), and new single-page windows pre-size from the image header before showing (`imageHeaderSize` + `presizedContentSize`). Result: small images open at 100% with the window hugging their edges, oversized images shrink to the screen, no first-open resize jump. Applies to reuse-window opens and page turns too.
- **Manual zoom decoupled from fit preference:** Pinch/manual zoom no longer cancels `alwaysFitOnOpen` (previously a single pinch permanently disabled fit-on-open, persisted to disk — Xee issue #415 class bug). `isManualZoom` is now session-scoped: kept across page turns within a folder (manga zoom reading), reset on `loadFolder` and app launch. Status bar `MANUAL xx%` disambiguates from the still-checked Always Fit menu item; manual-zoom hint now points to ⌘0.
